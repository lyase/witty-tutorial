...
*\b compiles & run  with : \code // rename testUser_Class.cpp User.hpp->testUser_Class.cpp g++ -g -lboost_unit_test_framework  -o tests testUser_Class.cpp User.cpp&& ./tests --log_level=test_suite --report_level=short \endcode
We run our tests with ./test --report_format=xml --report_level=detailed --log_level=test_suite --log_format=xml. You need both stderr and stdout, then we replace <TestLog> with <xml><TestLog> and replace </TestResult> with </TestResult><xml>. After that we run it though tidy and finally run that though this xslt.

./tests --log_format=XML --report_level=detailed 
or see http://www.boost.org/doc/libs/1_35_0/libs/test/doc/components/utf/parameters/index.html for more options running the tests
*\n
*\version  1.0
